...
void synch(float *timeEsp1, float *timeEsp2, int *equalCount1, int *equalCount2) {
  ...
  if ((*equalCount1 == 1) && (*equalCount2 == 1)) {
    ...
    if (hiWaikeUp.block  == 1) {
      ...
      /*************calulating epsilon***********************/
      float disych_Bloc = 1.0f;
      float epsilon2 ;
      if ( abs((timeRec21 - timeRec12) / 1000) <= TIME_TO_SLEEP) {

        epsilon2 = (timeRec21 - timeRec12) / 1000;
        if (epsilon2 >= 0) {
          epsilon =  -(epsilon2 - disych_Bloc);

        } else {
          epsilon = (epsilon2 + disych_Bloc);
        }
      } else if ( abs((timeRec21 - timeRec11) / 1000) <= TIME_TO_SLEEP) {

        epsilon2 = (timeRec21 - timeRec11) / 1000;
        if (epsilon2 >= 0) {
          epsilon =  -(epsilon2 - disych_Bloc);
      ...
      float disych_Bloc = 1.0f;
      float epsilon3 ;
      if ( abs((timeRec31 - timeRec22) / 1000) <= TIME_TO_SLEEP) {

        epsilon3 = (timeRec31 - timeRec22) / 1000;
        if (epsilon3 >= 0) {
        epsilon =  -(epsilon3 - disych_Bloc);
        }
        else {
          epsilon = (epsilon3 + disych_Bloc);
        }
      } else if ( abs((timeRec31 - timeRec21) / 1000) <= TIME_TO_SLEEP) {

        epsilon3 = (timeRec31 - timeRec21) / 1000;
        if (epsilon3 >= 0) {
          epsilon =  -(epsilon3 - disych_Bloc);
      ...
      } else if ( abs((timeRec31 - timeRec12) / 1000) <= TIME_TO_SLEEP) {

        epsilon3 = (timeRec31 - timeRec12) / 1000;
        if (epsilon3 >= 0) {
          epsilon =  -(epsilon3 - disych_Bloc);
      ...
      } else if ( abs((timeRec31 - timeRec11) / 1000) <= TIME_TO_SLEEP) {

        epsilon3 = (timeRec31 - timeRec11) / 1000;
        if (epsilon3 >= 0) {
          epsilon =  -(epsilon3 - disych_Bloc);
...
//    synch(&timeRec31, &timeRec32, &equalCountEsp31, &equalCountEsp32)
    ...
    if (controlPar.timeSleepEsp1  < 0 ) {
      controlPar.timeSleepEsp1  = 0;
    }
    if (controlPar.timeSleepEsp2  < 0 ) {
      controlPar.timeSleepEsp2  = 0;
    }
...
void spreadData1() {
  ...
  if (myMpuData.espID == 1) {
    ...
    */
    ...
    Serial.print(myMpuData.meanOmega); Serial.print(" , ");
